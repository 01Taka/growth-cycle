import { HistorySortType } from '../../../learningHistory/types/learning-history-types';
 ...
 */
export function filterCycleItems(
  data: CycleItemData[],
  ...
  return data.filter((item) => item.subject === subjectFilter);
}
 ...
 */
const compareByDifference = (a: CycleItemData, b: CycleItemData) => {
  const diffA = a.differenceToNextFixedReview;
  const diffB = b.differenceToNextFixedReview;
 ...
 */
export function sortCycleItems(
  data: CycleItemData[],
  sortBy: HistorySortType | null
): CycleItemData[] {
  if (!sortBy) {
    return data;
  }
  ...
  return [...data].sort((a, b) => {
    ...
    if (sortBy === 'fixation') {
      ...
      if (a.fixation !== b.fixation) {
        // 定着度 (昇順): 小さい値 (定着度が低い) を先頭に
        return a.fixation - b.fixation;
      }

      return b.differenceFromLastAttempt - a.differenceFromLastAttempt;
    }
    ...
    if (sortBy.startsWith('lastAttempt')) {
      ...
      if (sortBy === 'lastAttempt_desc') {
        ...
        result = b.differenceFromLastAttempt - a.differenceFromLastAttempt;
      } else {
        // 'lastAttempt_asc'
        // 昇順: 小さい値 (最近取り組んだ) を先頭に
        result = a.differenceFromLastAttempt - b.differenceFromLastAttempt;
      }

      // 同値の場合は定着度で昇順ソート
      return result !== 0 ? result : a.fixation - b.fixation;
    }
