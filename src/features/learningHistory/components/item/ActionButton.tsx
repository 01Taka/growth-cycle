  ...
  return (
    <Stack align="end" gap={4} w={80} style={{ flexShrink: 0 }}>
      <ActionIcon
        ...
        size={rem(40)}
        radius="xl"
        aria-label={LEARNING_HISTORY_ITEM_TEXTS.actionIconAriaLabel}
        style={{
      ...
      <Flex justify="end" gap={1}>
        <Text>
          {
            LEARNING_HISTORY_ITEM_TEXTS.testTimeLabel(testTargetProblemCount, estimatedTestTimeMin)
              .count
          }
        </Text>
        <Text>{LEARNING_HISTORY_ITEM_TEXTS.timeSeparator}</Text>
        <Text>
          {
            LEARNING_HISTORY_ITEM_TEXTS.testTimeLabel(testTargetProblemCount, estimatedTestTimeMin)
              .time
