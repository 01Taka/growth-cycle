import { ZodError } from 'zod'; // Zodを使っていると仮定
 ...
 * @returns 重複が削除された新しい配列 (idbIdが付与される)
 */
...
export const handleCreateTextbook = async (form: CreateTextbookForm): Promise<TextbookResult> => {
  try {
    const textbookPath = generateIdbPath(IDB_PATH.textbooks, '', true);

    // 1. 教科書オブジェクトの生成
    const textbook: Textbook = {
      name: form.name,
      subject: form.subject,
      units: assignId(form.units).map((unit) => ({ name: unit.text, id: unit.idbId })),
      categories: assignId(form.categories).map((category) => ({
        name: category.text,
        id: category.idbId,
        timePerProblem: 0,
    ...
    TextbookSchema.parse(textbook);

    // 3. IndexedDBへの追加 (IDB操作のエラーを投げる可能性あり)
